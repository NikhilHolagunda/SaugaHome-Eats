...
function OrderCard({ order, onAccept, onDecline, actionInFlight }) {
            ...
            {items.map(it => (
              <li key={it.id} className="flex justify-between text-sm">
                <span className="text-text-dark">
                  {it.quantity} × {it.name || `Item #${it.menu_item_id}`}
                </span>
...
export default function SellerOrdersPage() {
  ...
  const historical = orders.filter(o => o.status !== 'Placed');
    ...
    <div className="max-w-5xl mx-auto px-4 py-10 w-full">
      <div className="mb-8">
        ...
        <p className="text-text-muted">
          Accept incoming orders or decline them. Buyers see the status update immediately.
        </p>
      </div>
